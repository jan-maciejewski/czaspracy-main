                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form method="POST" action="{{ route('work-entries.update', $workEntry) }}">
                        ...
                        <div>
                            ...
                            <select id="user_id" name="user_id" class="block mt-1 w-full border-gray-300 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 focus:border-indigo-500 dark:focus:border-indigo-600 focus:ring-indigo-500 dark:focus:ring-indigo-600 rounded-md shadow-sm" required>
                                ...
                                @foreach ($employees as $employee)
                                    <option value="{{ $employee->id }}" {{ old('user_id', $workEntry->user_id) == $employee->id ? 'selected' : '' }}>
                        ...
                        <div class="mt-4">
                            ...
                            <x-text-input id="date_of_work" class="block mt-1 w-full" type="date" name="date_of_work" :value="old('date_of_work', $workEntry->date_of_work->format('Y-m-d'))" required />
                            <x-input-error :messages="$errors->get('date_of_work')" class="mt-2" />
                        </div>

                        <div class="mt-4">
                            <x-input-label for="hours_worked" :value="__('Przepracowane Godziny')" />
                            <x-text-input id="hours_worked" class="block mt-1 w-full" type="number" name="hours_worked" :value="old('hours_worked', $workEntry->hours_worked)" required step="0.01" min="0.01" max="24" />
